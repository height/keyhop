...
KeyHop 是 macOS 应用启动器。通过 `npm install -g keyhop` 安装，运行 `keyhop` 后，从菜单栏或每个 App 绑定的全局快捷键打开目标 App。代理默认关闭，需要时由用户手动开启；代理服务由已有的软件提供，KeyHop 不修改系统代理、不提供代理服务器。`proxy-launcher` 作为兼容命令保留。
