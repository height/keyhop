...
Usage:
  proxy-launcher             启动菜单栏；已有实例时打开设置
  proxy-launcher start       同上
  proxy-launcher status      查看菜单栏、代理检查及最近启动记录
  proxy-launcher status --json
  proxy-launcher quit        正常退出菜单栏（不会退出目标 App）
  proxy-launcher --help
  proxy-launcher --version

keyhop 是 proxy-launcher 的等价命令。代理默认关闭，可在设置中开启。
...
export function parseArguments(args) {
  ...
  throw new Error(`无法识别的命令或参数：${args.join(' ')}。运行 proxy-launcher --help 查看用法。`);
}
